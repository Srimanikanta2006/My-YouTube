    ...
    <div className="w-full h-screen bg-gray-50 dark:bg-zinc-950 flex flex-col md:flex-row overflow-hidden font-sans text-gray-800 dark:text-zinc-100 antialiased transition-colors">
      ...
      <div className="flex-1 bg-gray-50 dark:bg-zinc-950 overflow-y-auto p-2 sm:p-4 md:p-5 pt-2 sm:pt-4 md:pt-5 flex flex-col gap-2.5 min-w-0 h-full transition-colors">
        ...
        <div className="flex flex-col gap-2.5 flex-shrink-0 justify-start min-h-0">
          ...
          {/* Main video player container - sits tightly without top/bottom gaps */}
          <div className="w-full max-w-[1000px] mx-auto relative flex items-center justify-center my-0 py-0">
            {selectedVideo ? (
              <VideoPlayer
                video={selectedVideo}
              ...
              <div className="aspect-video w-full bg-zinc-900/90 rounded-2xl border border-zinc-800 flex flex-col items-center justify-center text-gray-400 space-y-2 p-6">
                <Users className="w-12 h-12 text-gray-500 animate-pulse" />
                <p className="font-semibold text-sm text-zinc-200">Waiting for Video Stream...</p>
                <p className="text-xs text-zinc-400">Only the host can select files from the library tray below.</p>
              </div>
            )}
          </div>
        ...
        <div className="w-full max-w-[1000px] mx-auto bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-3 shadow-sm items-center justify-between gap-3 shrink-0 hidden md:flex min-w-0 overflow-hidden">
          <div className="flex items-center gap-2 shrink-0">
            <h3 className="font-bold text-xs text-gray-900 dark:text-white shrink-0">Room Code:</h3>
            <span className="bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900/60 text-red-700 dark:text-red-300 px-2.5 py-1 rounded-lg text-xs font-mono select-all shrink-0">
              {roomId}
            </span>
            <Button
              onClick={handleCopyLink}
              size="sm"
              variant="outline"
              className="h-8 rounded-lg text-xs flex items-center gap-1 cursor-pointer bg-white dark:bg-zinc-800 border-gray-200 dark:border-zinc-700 text-zinc-800 dark:text-zinc-200 shrink-0"
            >
              {copied ? <Check className="w-3.5 h-3.5 text-green-600" /> : <Copy className="w-3.5 h-3.5" />}
          ...
          {isHost ? (
            <div className="flex items-center gap-2 min-w-0 flex-1 justify-end">
              <label className="text-xs font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-wider shrink-0">
                Stream Catalog:
              </label>
              {loadingVideos ? (
                <span className="text-xs text-gray-400 animate-pulse shrink-0">Loading...</span>
              ) : (
                <select
                  ...
                  className="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-gray-800 dark:text-zinc-100 rounded-xl px-3 py-1.5 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-red-600 w-full max-w-[200px] lg:max-w-[280px] min-w-0 truncate shadow-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-700 transition-colors"
                >
                  {videosList.map(v => (
                    <option key={v._id} value={v._id} className="truncate">
                      {v.videotitle}
                    </option>
                  ))}
                </select>
            ...
            selectedVideo && (
              <div className="text-xs min-w-0 flex-1 justify-end text-right truncate">
                <span className="text-gray-500 dark:text-zinc-400 shrink-0">Watching: </span>
                <span className="font-bold text-red-600 dark:text-red-400 truncate">{selectedVideo.videotitle}</span>
                <span className="text-gray-400 dark:text-zinc-500 font-normal ml-1.5 shrink-0">({selectedVideo.videochanel})</span>
              </div>
            )
          )}
        </div>
        ...
        <div className="w-full max-w-[1000px] mx-auto block lg:hidden pb-16">
          ...
          <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-3 shadow-sm mb-3 flex flex-col gap-2.5 md:hidden">
            ...
            {isHost ? (
              <div className="flex items-center justify-between gap-2 w-full pt-2 border-t border-gray-100 dark:border-zinc-800/80">
                <span className="text-xs font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-wider shrink-0">
                  Stream Catalog:
                </span>
                {loadingVideos ? (
                  ...
                  <select
                    ...
                    className="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-gray-800 dark:text-zinc-100 rounded-xl px-2.5 py-1 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-red-600 flex-1 min-w-0 truncate shadow-xs cursor-pointer"
                  >
                    {videosList.map(v => (
                      <option key={v._id} value={v._id} className="truncate">
                        {v.videotitle}
              ...
              selectedVideo && (
                <div className="text-xs pt-2 border-t border-gray-100 dark:border-zinc-800/80 truncate w-full flex items-center gap-1">
                  <span className="text-gray-500 dark:text-zinc-400 shrink-0">Watching:</span>
                  <span className="font-bold text-red-600 dark:text-red-400 truncate">{selectedVideo.videotitle}</span>
                  <span className="text-gray-400 dark:text-zinc-500 font-normal shrink-0">({selectedVideo.videochanel})</span>
                </div>
              )
            )}
          </div>
